...
import states.substates.ResultsSubState;
import ui.HealthBar;
import ui.TimeBar;
...
class PlayState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		strumLine = new FlxSprite(Options.save.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X,
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (controls.RESET && Options.save.data.resetKey && canReset && !inCutscene && startedCountdown && !endingSong)
		{
			health = 0;
	...
	override public function onFocus():Void
	{
		...
		if (health > 0 && !paused)
		{
			if (Conductor.songPosition > 0)
			{
				var endTimestamp:Float = FlxMath.bound(songLength - Conductor.songPosition - Options.save.data.noteOffset, 0);
	...
	override public function barHit(bar:Int):Void
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && Options.save.data.camZooms)
		{
	...
	public function startCountdown():Void
	{
		...
		#end
		{
			...
			startTimer = new FlxTimer().start(Conductor.beatLength / TimingConstants.MILLISECONDS_PER_SECOND, (tmr:FlxTimer) ->
			{
				...
				if (Options.save.data.showOpponentStrums)
				{
					notes.forEachAlive((note:Note) ->
					{
						if (note.mustPress)
			...
			case 'Philly Glow':
				...
				switch (lightId)
				{
					...
					case 1: // turn on
						...
						else if (Options.save.data.flashing)
						{
	...
	public function updateDirectionalCamera():Void
	{
		...
		if (camDirectional && Options.save.data.camFollowsAnims)
		{
