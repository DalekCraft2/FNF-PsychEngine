...
class CachingState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		logo = new FlxSprite(FlxG.width / 2, FlxG.height / 2).loadGraphic(Paths.getGraphic(Path.join(['ui', 'title', 'logo'])));
		logo.x -= logo.width / 2;
		...
		if (Options.save.data.cacheImages)
		{
			...
			for (file in Paths.fileSystem.readDirectoryRecursive(Path.join(['assets', 'images'])))
			{
