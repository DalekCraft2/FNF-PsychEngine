...
import util.XYStruct.CoordStruct;
...
class Stage
{
	...
	public function new(id:String)
	{
		...
		if (stageDef.groups != null)
		{
			for (groupDef in stageDef.groups)
			{
				...
				for (spriteDef in groupDef.objects)
				{
					...
					if (spriteDef.distraction && !Options.save.data.distractions)
					{
						continue;
					}
		...
		switch (id)
		{
			case 'philly':
				...
				blammedLightsBlack.antialiasing = Options.save.data.globalAntialiasing;
				...
				phillyWindowEvent.antialiasing = Options.save.data.globalAntialiasing;
				...
				phillyGlowGradient.antialiasing = Options.save.data.globalAntialiasing;
		...
		if (!Options.save.data.noStage)
		{
			switch (id)
			{
				...
				case 'schoolEvil':
					...
					if (bgGhouls != null)
					{
						if (!Options.save.data.lowQuality && bgGhouls.animation.finished)
						{
	...
	{
		if (beat % danceRate == 0 && Options.save.data.distractions && animatedLayers.length > 0)
		{
