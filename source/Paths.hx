...
class Paths
{
	...
	public static var dumpExclusions:Array<String> = [
		...
		Path.withExtension('shared:assets/shared/music/breakfast', AUDIO_EXT),
		Path.withExtension('shared:assets/shared/music/tea-time', AUDIO_EXT),
	];
	...
	private static function set_currentModDirectory(value:String):String
	{
		...
		currentModDirectory = value == null ? '' : value;
	...
	public static var fileSystem(get, default):IFileSystem;

	private static function get_fileSystem():IFileSystem
	{
		return FileSystemCP.getFileSystem();
	}

	public static function getPath(file:String):String
	{
		return Assets.getPath(file);
	}
	...
	public static function file(file:String, type:AssetType = TEXT, ?library:String):String
	{
		...
		// Debug.logTrace('file($file, $type, $library), currentModDirectory == $currentModDirectory');
	...
	{
		return music(Path.join(['songs', songId, 'Voices']), 'shared');
	}

	public static inline function inst(songId:String):String
	{
		return music(Path.join(['songs', songId, 'Inst']), 'shared');
	}
	...
	{
		FlxG.sound.load(getAudioDirect(path));
	}
	...
	public static function getModDirectories():Array<String>
	{
		...
		if (fileSystem.exists(modsFolder))
		{
			...
			{
				if (fileSystem.isDirectory(mods(folder)) && !list.contains(folder))
				{
					list.push(folder);
				}
			}
	...
	public static function getSortedModDirectories(omitDisabledMods:Bool = true):Array<String>
	{
		...
		if (exists(modListPath))
		{
			...
			for (mod in modList)
			{
				...
				{
					if (fileSystem.isDirectory(mods(directory))
	...
	public static function getDirectoryLoadOrder(onlyIncludeCurrentMod:Bool = false):Array<String>
	{
		...
		{
			directories = directories.concat([for (i in getSortedModDirectories()) mods(i)]);
		}
		#end
