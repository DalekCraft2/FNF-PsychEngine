...
class HitGraph extends Sprite
{
	...
	public var minValue:Float = -(Math.floor((PlayState.rep.replay.sf / TimingConstants.SECONDS_PER_MINUTE) * TimingConstants.MILLISECONDS_PER_SECOND) + 95);
	public var maxValue:Float = Math.floor((PlayState.rep.replay.sf / TimingConstants.SECONDS_PER_MINUTE) * TimingConstants.MILLISECONDS_PER_SECOND) + 95;

	public var showInput:Bool = Options.profile.inputShow;
	...
	public function new(x:Int, y:Int, width:Int, height:Int)
	{
		...
		ts = Math.floor((PlayState.rep.replay.sf / TimingConstants.SECONDS_PER_MINUTE) * TimingConstants.MILLISECONDS_PER_SECOND) / 166;
	...
	private function drawJudgementLine(ms:Float):Void
	{
		...
		var ts:Float = Math.floor((PlayState.rep.replay.sf / TimingConstants.SECONDS_PER_MINUTE) * TimingConstants.MILLISECONDS_PER_SECOND) / 166;
	...
	private function drawGraph():Void
	{
		...
		for (i in 0...history.length)
		{
			...
			var judge:Judgement = history[i][1];
