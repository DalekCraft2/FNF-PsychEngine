...
class Ratings
{
	...
	public static function judgeNote(noteDiff:Float):Judgement
	{
		...
		timingWindows = [Options.profile.sickWindow, Options.profile.goodWindow, Options.profile.badWindow];
			...
			accuracy:Float):String
	{
		...
		var npsString:String = Options.profile.npsDisplay ? 'NPS: $nps (Max $maxNPS)${showAll ? ' | ' : ''}' : ''; // NPS
		var scoreString:String = 'Score: ${Options.profile.safeFrames != OptionDefaults.safeFrames ? '$score ($scoreDefault)' : Std.string(score)}'; // Score
		...
		var fullAccuracyString:String = Options.profile.accuracyDisplay ? ' | $comboBreaksString | $accuracyString | $comboLetterRankString' : '';
