...
import funkin.MenuCharacter.MenuCharacterDef;
...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (char.animation.curAnim != null && char.animation.name == 'confirm' && char.animation.finished)
		{
	...
	private var curTypeSelected:Int = 0; // 0 = Opponent, 1 = BF, 2 = GF
	...
	private function reloadCharacter():Void
	{
		...
		char.alpha = 1;
		char.frames = Paths.getFrames(Path.join(['ui', 'story', 'menu_characters', characterFile.image]));
	...
	{
		var data:String = Json.stringify(characterFile, Constants.JSON_SPACE);
		if (data.length > 0)
		{
