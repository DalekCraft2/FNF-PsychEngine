...
import funkin.DialogueBoxPsych.DialogueAnimationDef;
import funkin.DialogueBoxPsych.DialogueCharacter;
import funkin.DialogueBoxPsych.DialogueCharacterDef;
...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		box.antialiasing = Options.profile.globalAntialiasing;
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (character.animation.curAnim != null)
		{
			...
			else if (character.animation.finished)
			{
		...
		if (!blockInput && !animationDropDown.dropPanel.visible)
		{
			...
			{
				character.playAnim(character.jsonFile.animations[curAnim].name);
				updateTextBox();
				reloadText();
			}
			...
			if (UI_mainbox.selected_tab_id != lastTab)
			{
				...
				else
				{
					...
					character.playAnim(character.jsonFile.animations[curAnim].name);
					animText.text = 'Animation: ${character.jsonFile.animations[curAnim].name} (${curAnim + 1} / ${character.jsonFile.animations.length}) - Press W or S to scroll';
				}
				...
				if (controlAnim.contains(true))
				{
					for (i in 0...controlAnim.length)
					{
						if (controlAnim[i] && character.jsonFile.animations.length > 0)
						{
							...
							var animToPlay:String = character.jsonFile.animations[curAnim].name;
	...
	private function addAnimationsUI():Void
	{
		...
		animationDropDown = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), (animation:String) ->
		{
			...
			if (character.dialogueAnimations.exists(anim))
			{
				...
				animationInputText.text = animShit.name;
				loopInputText.text = animShit.loopPrefix;
				idleInputText.text = animShit.idlePrefix;
			}
		});
		...
		var addUpdateButton:FlxButton = new FlxButton(10, idleInputText.y + 30, 'Add/Update', () ->
		{
			...
			if (character.dialogueAnimations.exists(theAnim)) // Update
			{
				...
				{
					if (animArray.name.trim() == theAnim)
					{
						animArray.loopPrefix = loopInputText.text;
						animArray.idlePrefix = idleInputText.text;
						break;
					}
				}
			...
			else // Add
			{
				var newAnim:DialogueAnimationDef = {
					name: theAnim,
					loopPrefix: loopInputText.text,
					loopOffsets: [0, 0],
					idlePrefix: idleInputText.text,
					idleOffsets: [0, 0]
				}
		...
		var removeUpdateButton:FlxButton = new FlxButton(100, addUpdateButton.y, 'Remove', () ->
		{
			for (animArray in character.jsonFile.animations)
			{
				if (animArray != null && animArray.name.trim() == animationInputText.text.trim())
				{
					var lastSelected:String = animationDropDown.selectedLabel;
					...
					ghostIdle.reloadAnimations();
					reloadAnimationsDropDown();
					if (character.jsonFile.animations.length > 0 && lastSelected == animArray.name.trim())
					{
						var animToPlay:String = character.jsonFile.animations[0].name;
	...
	private function reloadCharacter():Void
	{
		...
		{
			char.frames = Paths.getFrames(Path.join(['dialogue', 'portraits', character.jsonFile.image]));
			char.jsonFile = character.jsonFile;
			char.reloadAnimations();
			var scaleValue:Float = DialogueCharacter.DEFAULT_SCALE * character.jsonFile.scale;
		...
		character.playAnim(character.jsonFile.animations[0].name);
		if (character.jsonFile.animations.length > 0)
		{
			curSelectedAnim = character.jsonFile.animations[0].name;
			var animShit:DialogueAnimationDef = character.dialogueAnimations.get(curSelectedAnim);
			ghostLoop.playAnim(animShit.name);
			ghostIdle.playAnim(animShit.name, true);
			offsetLoopText.text = 'Loop: ${animShit.loopOffsets}';
			offsetIdleText.text = 'Idle: ${animShit.idleOffsets}';
		}
	...
	{
		var data:String = Json.stringify(character.jsonFile, Constants.JSON_SPACE);
		if (data.length > 0)
		{
