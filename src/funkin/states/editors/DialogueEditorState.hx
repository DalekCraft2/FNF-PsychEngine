...
import funkin.DialogueBoxPsych.DialogueLineDef;
...
class DialogueEditorState extends MusicBeatState
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (character.animation.curAnim != null)
		{
			...
			{
				if (character.animationIsLoop() && character.animation.finished)
				{
		...
		if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			if (sender == characterInputText)
			{
				...
				if (character.jsonFile.animations.length > 0)
				{
					...
					{
						character.playAnim(character.jsonFile.animations[curAnim].name, text.finishedText);
	...
	{
		character.frames = Paths.getFrames(Path.join(['dialogue', 'portraits', character.jsonFile.image]));
		character.jsonFile = character.jsonFile;
		character.reloadAnimations();
		var scaleValue:Float = DialogueCharacter.DEFAULT_SCALE * character.jsonFile.scale;
		character.scale.set(scaleValue, scaleValue);
	...
	private function changeText(change:Int = 0):Void
	{
		...
		if (animsLength > 0)
		{
			for (i in 0...animsLength)
			{
				...
				if (animation != null && animation.name == curDialogue.expression)
				{
					curAnim = i;
					break;
				}
			...
			character.playAnim(character.jsonFile.animations[curAnim].name, text.finishedText);
			animText.text = 'Animation: ${character.jsonFile.animations[curAnim].name} (${(curAnim + 1)} / $animsLength) - Press W or S to scroll';
		}
		else
		{
			animText.text = 'ERROR! NO ANIMATIONS FOUND';
	...
	{
		var data:String = Json.stringify(dialogueFile, Constants.JSON_SPACE);
		if (data.length > 0)
		{
