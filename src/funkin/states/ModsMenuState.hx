...
class ModsMenuState extends MusicBeatState implements ListMenu
{
	...
	override public function create():Void
	{
		...
		buttonDisableAll = new FlxButton(startX, 620, 'Disable All', () ->
		{
		...
		startX -= 190;
		buttonEnableAll = new FlxButton(startX, buttonDisableAll.y, 'Enable All', () ->
		{
			for (modEnableState in modList)
			{
		...
		#if sys
		startX -= 190;
		installButton = new FlxButton(startX, buttonEnableAll.y, 'Install Mod', () ->
		{
			fileBrowseDialog();
		});
		...
		startX -= 190;
		uninstallButton = new FlxButton(startX, installButton.y, 'Uninstall Selected Mod', () ->
		{
			uninstallMod();
		});
	...
	private function moveMod(change:Int, skipResetCheck:Bool = false):Void
	{
		if (mods.length > 1)
		{
			...
			var newPos:Int = FlxMath.wrap(curSelected + change, 0, mods.length - 1);
			var lastModEnableState:ModEnableState = modList[curSelected];
			modList.remove(lastModEnableState);
			modList.insert(newPos, lastModEnableState);

			var lastMod:ModMetadata = mods[curSelected];
			mods.remove(lastMod);
			mods.insert(newPos, lastMod);
