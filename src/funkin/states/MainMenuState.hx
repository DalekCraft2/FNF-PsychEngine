...
import funkin.states.editors.MasterEditorMenuState;
import haxe.io.Path;
import openfl.Lib;
#if FEATURE_DISCORD
...
class MainMenuState extends MusicBeatState implements ListMenu
{
	...
	override public function create():Void
	{
		...
		debugKeys = Options.copyKey(Options.profile.keyBinds.get('debug_1'));
		...
		// var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.getGraphic(Path.join(['ui', 'main', 'backgrounds', 'menuDesat'])));
		// bg.color = 0xFFFFEA72; // Tint used to get menuBG from menuDesat (or, at least, it is close to what the tint is)
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.getGraphic(Path.join(['ui', 'main', 'backgrounds', 'menuBG'])));
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.getGraphic(Path.join(['ui', 'main', 'backgrounds', 'menuDesat'])));
		magenta.scrollFactor.set(0, yScroll);
		magenta.scale.set(1.175, 1.175);
		magenta.updateHitbox();
		magenta.screenCenter();
		magenta.visible = false;
		magenta.antialiasing = Options.profile.globalAntialiasing;
		...
		for (i => menuOption in menuOptions)
		{
			...
			menuItem.frames = Paths.getFrames(Path.join(['ui', 'main', 'options', menuOption]));
			...
			menuItem.antialiasing = Options.profile.globalAntialiasing;
