...
class SoundOffsetState extends MusicBeatState
{
	...
	public var currOffset:Int = Options.profile.noteOffset;
	...
	override public function create():Void
	{
		...
		// var menuBG:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.getGraphic(Path.join(['ui', 'main', 'backgrounds', 'menuDesat'])));
		// menuBG.color = 0xFFFFEA72; // Tint used to get menuBG from menuDesat (or, at least, it is close to what the tint is)
		var menuBG:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.getGraphic(Path.join(['ui', 'main', 'backgrounds', 'menuBG'])));
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (FlxG.keys.justPressed.SPACE)
		{
			...
			if (!playingAudio)
			{
				Options.profile.noteOffset = currOffset;
			}
		}
