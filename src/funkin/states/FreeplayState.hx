...
class FreeplayState extends MusicBeatState implements ListMenu
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		else if (space && !selectedSong)
		{
			if (instPlaying != curSelected)
			{
				...
				// FIXME This doesn't seem to respect needsVoices properly because the PlayState song is not set first
				// if (PlayState.song.needsVoices)
				if (Paths.exists(Paths.voices(songId)))
	...
	{
		songs.push(SongMetadata.getSongMetadata(songId, week));
	}
