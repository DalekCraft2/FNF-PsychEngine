...
class NoteOffsetState extends MusicBeatState
{
	...
	public function new()
	{
		...
		if (Options.profile.comboOffset == null)
			Options.profile.comboOffset = [0, 0, 0, 0];
	}
	...
	override public function create():Void
	{
		...
		FlxG.sound.pause();
		// Stage
		var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.getGraphic(Path.join(['stages', 'stage', 'stageback'])));
		bg.scrollFactor.set(0.9, 0.9);
		bg.antialiasing = Options.profile.globalAntialiasing;
		add(bg);

		var stageFront:FlxSprite = new FlxSprite(-650, -600).loadGraphic(Paths.getGraphic(Path.join(['stages', 'stage', 'stagefront'])));
		...
		{
			var stageLight:FlxSprite = new FlxSprite(-125, -100).loadGraphic(Paths.getGraphic(Path.join(['stages', 'stage', 'stage_light'])));
			stageLight.scrollFactor.set(0.9, 0.9);
			stageLight.scale.set(1.1, 1.1);
			stageLight.updateHitbox();
			stageLight.antialiasing = Options.profile.globalAntialiasing;
			add(stageLight);

			var stageLight2:FlxSprite = new FlxSprite(1225, -100).loadGraphic(Paths.getGraphic(Path.join(['stages', 'stage', 'stage_light'])));
			stageLight2.scrollFactor.set(0.9, 0.9);
			stageLight2.scale.set(1.1, 1.1);
			stageLight2.updateHitbox();
			stageLight2.antialiasing = Options.profile.globalAntialiasing;
			stageLight2.flipX = true;
			add(stageLight2);

			var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.getGraphic(Path.join(['stages', 'stage', 'stagecurtains'])));
			stageCurtains.scrollFactor.set(1.3, 1.3);
			stageCurtains.scale.set(0.9, 0.9);
			stageCurtains.updateHitbox();
			stageCurtains.antialiasing = Options.profile.globalAntialiasing;
			add(stageCurtains);
		}
		...
		rating = new FlxSprite().loadGraphic(Paths.getGraphic(Path.join(['ui', 'judgements', 'sick'])));
		...
		{
			var numScore:FlxSprite = new FlxSprite(43 * loopsDone).loadGraphic(Paths.getGraphic(Path.join(['ui', 'combo', 'num$i'])));
		...
		barPercent = Options.profile.noteOffset;
		updateNoteDelay();

		timeBarBG = new FlxSprite(0, timeTxt.y + 8).loadGraphic(Paths.getGraphic(Path.join(['ui', 'hud', 'timeBar'])));
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (onComboMenu)
		{
			...
			if (controlArray.contains(true))
			{
				for (i in 0...controlArray.length)
				{
					if (controlArray[i])
					{
						...
						{
							case 0:
								Options.profile.comboOffset[0] -= addNum;
							case 1:
								Options.profile.comboOffset[0] += addNum;
							case 2:
								Options.profile.comboOffset[1] += addNum;
							case 3:
								Options.profile.comboOffset[1] -= addNum;
							case 4:
								Options.profile.comboOffset[2] -= addNum;
							case 5:
								Options.profile.comboOffset[2] += addNum;
							case 6:
								Options.profile.comboOffset[3] += addNum;
							case 7:
								Options.profile.comboOffset[3] -= addNum;
						}
					}
			...
			if (FlxG.mouse.justPressed)
			{
					...
					&& startMousePos.y - comboNums.y <= comboNums.height)
				{
					...
					startComboOffset.x = Options.profile.comboOffset[2];
					startComboOffset.y = Options.profile.comboOffset[3];
				}
					...
					&& startMousePos.y - rating.y <= rating.height)
				{
					...
					startComboOffset.x = Options.profile.comboOffset[0];
					startComboOffset.y = Options.profile.comboOffset[1];
				}
			}
			...
			if (holdingObjectType != null)
			{
				if (FlxG.mouse.justMoved)
				{
					...
					Options.profile.comboOffset[addNum + 0] = Math.round((mousePos.x - startMousePos.x) + startComboOffset.x);
					Options.profile.comboOffset[addNum + 1] = -Math.round((mousePos.y - startMousePos.y) - startComboOffset.y);
					repositionCombo();
				}
			}
			...
			if (controls.RESET)
			{
				for (i in 0...Options.profile.comboOffset.length)
				{
					Options.profile.comboOffset[i] = 0;
				}
				repositionCombo();
			}
		}
		else
		{
			...
			{
				barPercent = Math.max(delayMin, Math.min(Options.profile.noteOffset - 1, delayMax));
				updateNoteDelay();
			}
	...
	override public function beatHit(beat:Int):Void
	{
		...
		if (beat % Conductor.BEATS_PER_BAR == 2)
		{
	...
	private function repositionCombo():Void
	{
		...
		comboNums.x = coolText.x - 90 + Options.profile.comboOffset[2];
		comboNums.y += 80 - Options.profile.comboOffset[3];
		reloadTexts();
	}
	...
	private function reloadTexts():Void
	{
		for (i => text in dumbTexts.members)
		{
			switch (i)
			{
				...
				case 1:
					text.text = '[${Options.profile.comboOffset[0]}, ${Options.profile.comboOffset[1]}]';
				case 2:
					text.text = 'Numbers Offset:';
				case 3:
					text.text = '[${Options.profile.comboOffset[2]}, ${Options.profile.comboOffset[3]}]';
			}
		}
	}
	...
	{
		Options.profile.noteOffset = Math.round(barPercent);
