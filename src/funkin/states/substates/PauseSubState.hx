...
class PauseSubState extends MusicBeatSubState implements ListMenu
{
	...
	override public function create():Void
	{
		...
		if (pauseMusic == null || !pauseMusic.playing)
		{
			...
			{
				pauseMusic.loadEmbedded(Paths.getMusic(Paths.formatToSongPath(Options.profile.pauseMusic)), true, true);
			}
			pauseMusic.volume = 0;
		...
		if (accepted)
		{
			...
			switch (selectedOption)
			{
				...
				case 'Exit to Menu':
					...
					{
						Options.profile.botPlay = false;
						Options.profile.scrollSpeed = 1;
						Options.profile.downScroll = false;
					}
