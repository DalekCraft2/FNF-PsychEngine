...
class OptionsSubState extends MusicBeatSubState implements ListMenu
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (leftP || rightP)
		{
			...
			if (option is IntegerOption || option is FloatOption)
			{
				holdValue = Reflect.getProperty(option, 'value');
			}
		}

		if (holdTime > 0.5 && (left || right) && (option is IntegerOption || option is FloatOption))
		{
			var option:ArrowOption<OneOfTwo<Int, Float>> = cast option;
			var scrollSpeed:OneOfTwo<Int, Float> = 0;
			var min:OneOfTwo<Int, Float> = 0;
			var max:OneOfTwo<Int, Float> = 0;
			if (option is IntegerOption)
			{
				var option:IntegerOption = cast option;
			...
			{
				var option:FloatOption = cast option;
				scrollSpeed = option.scrollSpeed;
				min = option.min;
				max = option.max;
			}
			holdValue = FlxMath.bound(holdValue + scrollSpeed * elapsed * (left ? -1 : 1), min, max);
			if (option is IntegerOption)
			{
				option.value = Math.round(holdValue);
			}
			else if (option is FloatOption)
			{
				var option:FloatOption = cast option;
				var decimals:Int = option.decimals;
				if (decimals >= 0)
					option.value = FlxMath.roundDecimal(holdValue, decimals);
				else
					option.value = holdValue;
			}
		...
		if (option.allowMultiKeyInput)
		{
			...
			if (pressed != NONE)
			{
				option.keyPressed(pressed);
			}
			if (released != NONE)
			{
				option.keyReleased(released);
			}
		}
		if (accepted)
		{
		...
		{
			var option:ControlOption = cast option;
	...
	public function createDefault():Void
	{
		defCat = new OptionCategory('Default', [
			new OptionCategory('Gameplay', [
				new OptionCategory('Controls',
					[
						...
						{
							InitState.muteKeys = Options.copyKey(Options.profile.keyBinds.get('volume_mute'));
							FlxG.sound.muteKeys = InitState.muteKeys;
						}),
						new ControlOption('volume_up', controls, (keys:Array<FlxKey>) ->
						{
							InitState.volumeUpKeys = Options.copyKey(Options.profile.keyBinds.get('volume_up'));
							FlxG.sound.volumeUpKeys = InitState.volumeUpKeys;
						}),
						new ControlOption('volume_down', controls,
							(keys:Array<FlxKey>) ->
							{
								InitState.volumeDownKeys = Options.copyKey(Options.profile.keyBinds.get('volume_down'));
				...
				new FloatOption('scrollSpeed', 'Scroll Speed', 0.1, 0.5, (Options.profile.scrollType == 'multiplicative' ? 3 : 6),
					(Options.profile.scrollType == 'multiplicative' ? 'X' : '')),
			...
			new OptionCategory('Appearance', [
				...
				new BooleanOption('showOpponentStrums', 'Show Opponent Strums', 'Toggle whether the opponent\'s strums are shown'),
				...
				new FloatOption('bgAlpha', 'BG Opacity', 'How opaque the background is.', 10, 0, 100, '%', '', 2),
			...
			new OptionCategory('Preferences', [
				...
				new FloatOption('healthBarAlpha', 'Health Bar Opacity', 'How opaque the health bar and icons should be.', 0.1, 0, 1, 2),
				...
				new StringOption('pauseMusic', 'Pause Screen Song', 'The song which plays in the pause menu.', 0, 2, ['None', 'Breakfast', 'Tea Time'],
					(index:Int, name:String, indexAdd:Int) ->
					{
						...
						{
							if (name == 'None')
								PauseSubState.pauseMusic.stop();
							else
							{
								PauseSubState.pauseMusic.loadEmbedded(Paths.getMusic(Paths.formatToSongPath(name)));
				...
				new FloatOption('hitSoundVolume', 'Hit Sound Volume', 'The volume used for the hit sounds.', 10, 0, 100, '%', '', 2),
				new BooleanOption('fastTransitions', 'Fast Transitions', 'Makes transitions between states faster.')
			]),
			new OptionCategory('Performance', [
				new IntegerOption('frameRate', 'FPS Cap', 'The FPS the game tries to run at.', 30, 30, 360, '', '',
					(value:Float, step:Float) ->
					{
	...
	private function refresh():Void
	{
		...
		for (i in 0...category.options.length)
		{
			...
			category.options[i].createOptionText(curSelected, optionText);
			var text:Alphabet = category.options[i].text;
			text.targetY = i;
		}
